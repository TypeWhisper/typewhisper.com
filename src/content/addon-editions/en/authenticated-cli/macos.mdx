...
For OpenCode, TypeWhisper loads the Zen catalog through `opencode models opencode --verbose --pure`. It exposes only `opencode/*` text models whose complete pricing object includes input and output prices and whose every numeric value—including cache and tier pricing—is zero. Free offers are dynamic and can change or end at any time; TypeWhisper never falls back to an unverified or paid model. Safe variants reported for the selected model become its available effort levels on macOS.
