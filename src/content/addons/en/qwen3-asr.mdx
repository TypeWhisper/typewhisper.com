...
author: TypeWhisper
icon: Cpu
description: Qwen3-ASR locally through MLX on macOS or ONNX Runtime on Windows.
categories:
  - transcription
platforms:
  - mac
...
Qwen3 ASR is TypeWhisper's bundled local speech-to-text provider for Qwen3-ASR on Apple Silicon. Version `1.1.8` includes optional HuggingFace token storage, quick model picks, and ten MLX model variants from compact 0.6B quantized downloads to the larger 1.7B BF16 model.
...
Click **Download & Load** to download and activate a model. The loaded model becomes the active transcription model. Click **Unload** to release the model from memory. Downloaded files remain available; removing a download is a separate action.
...
2. Click **Unload** on the active model to release its memory without deleting the download.
...
- The public `1.1.8` release requires TypeWhisper host version `1.6.0` or newer.
