...
version: "0.2.1"
icon: Plug
description: Connect TypeWhisper workflows to MCP tools over stdio or Streamable HTTP and turn dictated or selected text into fixed actions.
categories:
  - action
platforms:
  - mac
...
MCP Client turns tools from [Model Context Protocol (MCP)](https://modelcontextprotocol.io/docs/getting-started/intro) servers into TypeWhisper workflow actions. Add a reusable stdio or Streamable HTTP server, discover its tools, and create one or more named actions that bind workflow text and context to a fixed tool.
...
Version `0.2.1` requires TypeWhisper `1.6.0` or newer on macOS.
...
## Set up Streamable HTTP

Choose **Streamable HTTP** when adding a server and enter its MCP endpoint URL. Remote endpoints require HTTPS; plain HTTP is allowed only for loopback addresses such as `localhost`. Select **Bearer token** if needed and save the token in macOS Keychain. Then select **Connect & Load Tools** and bind discovered tools to fixed workflow actions as in the stdio example.

HTTP calls send the mapped arguments to the selected server over the network. A local stdio process can also contact remote services itself.

## Current Limits

- Local stdio servers and Streamable HTTP are supported; legacy HTTP/SSE transports are not.
- Streamable HTTP supports a manually configured bearer token. OAuth and browser-based sign-in flows are not supported.
